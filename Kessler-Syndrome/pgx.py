...
def spriteSheetBreaker(sheet, width, height, margin, vertmargin, rows, columns):
    ...
    for i in range(rows):
        for j in range(columns[i]):
            image_list.append(
                sheet.subsurface(
                    ((width + margin) * j, (height + vertmargin) * i, width, height)
                )
            )
    return image_list
...
class Font:
    ...
    def splitSheet(fontsheet):
        Font.char_list = spriteSheetBreaker(
            Font.fontsheet, 8, 13, 2, 1, 6, [10, 10, 10, 6, 10, 4]
        )
    ...
    def getChar(char, scale):
        if Font.SCRAMBLED:
            charImage = scaleImage(
                Font.char_list[random.randint(0, len(Font.char_list) - 1)], scale
            )
        ...
        if inputvar != last_input:
            for i in range(len(inputvar)):
                ...
                if inputtype == "int":
                    ...
                    if (
                        len(inputvar[i]) == 1
                        and inputvar[i] in InputGetter.ALPHABETCHECK
                    ):
                        self.rawtext += inputvar[i]
...
class AnnouncementBox:
    ...
    def __init__(self, imagepath, soundpath, text):
        ...
        self.image = stretchImage(
            image, (108 * Texthelper.scalar, 108 * Texthelper.scalar)
        )
        self.sound = sound
        self.text = text
    ...
    def play(screen):
        ...
        if (
            AnnouncementBox.upcoming != []
        ):  ##entire entry can be deleted from draw, so this test actually is necessary
    ...
    def _draw(self, screen):
        ...
        for line in range(len(self.linedtext)):
            start = max([0, self.bounds[0] - total_chars])
                ...
                [len(self.linedtext[line]), max([0, self.bounds[1] - total_chars])]
            )
        ...
        if not self.printing:
            Texthelper.write(
                screen, [(768, 120 + boxheight), "Press Enter to Continue", 1.5]
            )
            inputvar = keydowns()
    ...
    def _timehelper(self):
        ...
        inputvar = keydowns()
        if (
            self.printing and pygame.K_RETURN in inputvar
        ):  # allows you to skip by pressing enter
...
class Texthelper:
    ...
    def _interpretcoords(text_input):
        ...
        if isinstance(location_list[0], str) and (
            "-" in location_list[0] or "+" in location_list[0]
        ):  # EX: right-110
            ...
            if location_list[0][index - 1] == "+":
                ...
                raise ValueError(
                    "you appear to have entered an incorrect coordinate format"
                )
        elif isinstance(
            location_list[0], str
        ):  # EX: 'center', 'center.600', 'left.20', 'right'
            if "center" in location_list[0]:
                if location_list[0][-1].isdigit():
                    ...
                    location_list[0] = (
                        Texthelper.width / 2 - Texthelper._textlength(text_input) / 2
                    )
                ...
                if location_list[0][-1].isdigit():
                    ...
                    location_list[0] = Texthelper.width - Texthelper._textlength(
                        text_input
                    )
            else:
    ...
    def _buttonlogic(rect):
        ...
        )  # // = Java integer math - IE truncating the ms to 1/5 seconds
        if time != Texthelper.lastPressTime:
            if AllEvents.contains(pygame.MOUSEBUTTONDOWN):
                if (
                    rect[0] < click[1] < rect[0] + rect[2]
                    and rect[1] < click[2] < rect[1] + rect[3]
                ):
    ...
    def writeBox(screen, text_input, **kwargs):
        ...
        boxrect = [
            x - padding,
            y - padding / 2,
            Texthelper._textlength(text_input) + padding * 2,
            13 * text_input[2] + padding,
        ]
    ...
    def writeNullButton(screen, text_input):
        ...
        click = mouse()
        if text_location[0] < click[1] < (text_location[0] + x_range) and text_location[
            1
        ] < click[2] < (text_location[1] + y_range):
            return True
        elif click == (0, 0, 0):
            return True
        else:
...
class Filehelper:
    ...
    def get(self, line):  # line is line # in file being extracted
        ...
        for i in range(
            len(parse_line)
        ):  # goes through the list guessing the types of the elements
    ...
    def set(self, content, line, **kwargs):  # line is line # in file being written to
        ...
        if (
            "override" in kwargs
        ):  # tests for optional argument that ditches any and all comments
...
filehelper = Filehelper(
    "saves/gamedata.txt"
)  # makes lowercase filehelper used throughtout work with the class
