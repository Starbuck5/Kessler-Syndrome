...
class Images:
    ...
    def addRotate(
        ID, image, **kwargs
    ):  # takes an ID and a surface and adds the dictionary of its rotations to Images storage
        image = Images._processImage(image, **kwargs)
        rotatedict = {}
        rectdict = {}
        for j in range(36):
    ...
    def getHitbox(
        xpos, ypos, name, rotation, centered=True, beFancy=True, realRotation=False
    ):
        ...
        if centered:
            return bound.move(
                xpos - 0.5 * image.get_width(), ypos - 0.5 * image.get_height()
            )
...
def init(d_asteroids, d_parts, d_sats, graphlist, scalar2, scalar3, scalarscalar):
    ...
    Images.add(
        "fuelpic", scaleImage(loadImage("images/fuelcanister.tif"), 2 * scalarscalar)
    )
    Images.add("armorpic", scaleImage(loadImage("images/armor.tif"), scalarscalar))
    Images.add(
        "shotpic",
        scaleImage(loadImage("images/missile.png"), scalarscalar),
    ...
    Images.add(
        "pygamebadge",
        scaleImage(
            loadImage("images/pygame-badge-SMA-unscaled.png"), 2.5 * scalarscalar
        ),
    )
    ...
    Images.addRotate(
        7, scaleImage(loadImage("images/alienMines.tif"), 2 * scalarscalar)
    )
    Images.add(9, scaleImage(loadImage("images/ionblast.png"), 0.5 * scalarscalar))

    # aliens
    Images.addRotate(
        120,
        scaleImage(loadImage("images/aliendrone.gif"), 2 * scalarscalar),
        colorkey=(255, 255, 255),
    )
    Images.addRotate(
        121,
        scaleImage(loadImage("images/spiker.gif"), 2 * scalarscalar),
        colorkey=(255, 255, 255),
    )
    Images.addRotate(
        122,
        scaleImage(loadImage("images/alienshot.gif"), scalarscalar),
        colorkey=(255, 255, 255),
    )
    # aliens - alien mines
    imageList = spriteSheetBreaker(
        loadImage("images/alienbomb.gif"), 19, 19, 0, 0, 1, 6
    )
    ...
    Images.add(
        101, change_color(base_star, (255, 216, 0, 255), (255, 160, 0, 255), True)
    )
    Images.add(
        102, change_color(base_star, (255, 216, 0, 255), (255, 130, 0, 255), True)
    )
    base_star = scaleImage(base_star_unscaled, 2 * scalarscalar)
    Images.add(103, base_star)
    Images.add(
        104, change_color(base_star, (255, 216, 0, 255), (255, 160, 0, 255), True)
    )
    Images.add(
        105, change_color(base_star, (255, 216, 0, 255), (255, 130, 0, 255), True)
    )
    ...
    Images.addRotate(
        666,
        scaleImage(loadImage("images/president.png"), scalarscalar),
        colorkey=(255, 255, 255),
    )
    ...
    for i in range(3):
        for j in range(0, len(object_list), 8):
            ...
            if object_number in background and i == 0:
                ...
                object_number not in ship and object_number not in background and i == 2
            ):
                newObject_list += object_list[j : j + 8]
...
def crayprinter(
    ...
    if object_number == 0:  # draws zvezda
        ...
        screen.blit(
            image,
            (int(xpos - 0.5 * image.get_width()), int(ypos - 0.5 * image.get_height())),
        )
        colliderect = [
        ...
        if flame == True:
            ...
            flame_pointlist = Rotate(
                xpos, ypos, flame_pointlist, rotation.getRotation()
            )
            flame_color = (
                (255, 100, 0) if pgx.filehelper.get(3)[4] < 1 else (138, 43, 226)
            )
        ...
        object_number == 2 or object_number == 8
    ):  # draws missiles (id 8 are alien missiles)
        ...
        try:
            ...
            colliderect = Images.getHitbox(
                xpos, ypos, object_number, rotation.getRotation()
            )
        except:
            pass
...
class InfoBars:
    ...
    def draw(
        ...
        pgx.draw.rect(
            screen, (139, 0, 0), ["right-220", 1000, 200 * currentfuel / totalfuel, 50]
        )
        ...
        Texthelper.write(
            screen,
            [("right-205", 865), str(ammunition) + "/" + str(totalammunition), 3],
        )
...
def drawSector(screen, location, number, currentsector, cleared):
    ...
    pgx.draw.rect(
        screen,
        color,
        (location[0] - secsize / 2, location[1] - secsize / 2, secsize, secsize),
        4,
    )

    if number == currentsector:
        Texthelper.write(
            screen, [(location[0] - 35, location[1] - 35), "U R Here", 1], color=color
        )
    Texthelper.write(
        screen,
        [(location[0] - len(str(number)) * 10, location[1] - 15), str(number), 2],
        color=color,
    )
...
def drawInventory(screen, shipInventory):
    Texthelper.write(
        screen, [("left+10", 10), f"metal:{shipInventory[0]}", 3], color=(120, 120, 120)
    )
    Texthelper.write(
        screen, [("left+315", 10), f"gas:{shipInventory[1]}", 3], color=(185, 20, 20)
    )
    Texthelper.write(
        screen,
