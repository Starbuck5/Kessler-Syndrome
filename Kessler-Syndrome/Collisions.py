...
def getHitbox(object_list, object_location, graphlist):
    ...
    if objectID == 1 or objectID == 5:  # main ship
        ...
        hitBox = graphics.Images.getHitbox(
            xpos, ypos, 1.1, -rotation.getRotation(), True, True, True
        )
        ...
        hitBox = graphics.Images.getHitbox(
            xpos, ypos, objectID, rotation.getRotation(), False
        )
        ...
        scale = 1 + (0.1 * (300 - object_list[object_location * 8 + 7]))
        hitBox = graphics.Images.getHitbox(xpos, ypos, objectID, rotation.getRotation())
        ...
        try:
            hitBox = graphics.Images.getHitbox(
                xpos, ypos, objectID, rotation.getRotation()
            )
        except:
            pass
    return hitBox
