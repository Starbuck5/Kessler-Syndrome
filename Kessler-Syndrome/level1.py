...
def level1(screen, width, height, scalarscalar, clock):
    ...
    mountains = pygame.transform.scale(
        mountains, (width, int(width / mountainsize[0] * mountainsize[1]))
    )
    ...
    while running:
        ...
        for i in range(len(opening_crawl)):
            Texthelper.write(
                screen, [("center", ypos + line_spacing * i), opening_crawl[i], 3]
            )

        draw.sblit(
            screen, soyuz, (xpos, ypos + line_spacing * len(opening_crawl) + 100)
        )
        draw.sblit(
            screen,
            randomflame(ypos),
            (xpos - 4, ypos + unsh + line_spacing * len(opening_crawl) + 100),
        )
        draw.sblit(
            screen,
            randomflame(ypos),
            (xpos + 6, ypos + unsh + line_spacing * len(opening_crawl) + 100),
        )
        draw.sblit(
            screen,
            randomflame(ypos),
            (xpos + 10, ypos + unsh + line_spacing * len(opening_crawl) + 100),
        )
        draw.sblit(
            screen,
            randomflame(ypos),
            (xpos + 20, ypos + unsh + line_spacing * len(opening_crawl) + 100),
        )
        ...
        if not (ypos + line_spacing * len(opening_crawl) + 400 > height / 2 - 100):
            ...
            if len(namebox.getText()) > 11:
                Texthelper.write(
                    screen,
                    [("center", 570), "name should be less than 12 characters", 1],
                )
            isValidInput = namebox.getText() != "name" and len(namebox.getText()) < 12
            isPressed = Texthelper.writeButton(
                screen, [("center", 540), "then press here or enter", 2]
            )
