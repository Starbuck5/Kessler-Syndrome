...
class AITools:
    ...
    def releaseMine(object_list, self_loc, angle):
        ...
        object_list += [
            ...
            ymom,
            123,
            RotationState(random.randint(0, 360), random.randint(-10, 10)),
            AlienMineAI(),
            1,
        ]
...
class DroneAI:
    ...
    def shooting(self, screen, object_list, self_loc, xpos, ypos):
        ...
        if self.shotCounter % 25 == 0:
            AITools.shoot(
                ...
                object_list,
                self_loc,
                object_list[self_loc + 5].getRotation(),
                122,
            )
        self.shotCounter += 1
        return droneShip
    ...
    def update(self, screen, object_list, self_loc):
        ...
        if self.progression == 0:
            ...
            if distance <= 100:
                self.progression = 3
            elif distance <= 300 and (
                rotationDistance < 10 or 360 - rotationDistance < 10
            ):
                self.progression = 1
...
class PrezAI(DroneAI, ArmorManager):
    ...
    def update(self, screen, object_list, self_loc):
        ...
        if self.progression == 0:
            ...
            xpos += xChange
            ypos += yChange
            droneShip = DroneAI.shooting(
                self, screen, object_list, self_loc, xpos, ypos
            )
            if self.shotCounter == 100:
                self.shotCounter = 0
                self.progression = 2
        ...
        pgx.Texthelper.write(
            screen, [("center", 70), "President of the World", 2], color=(110, 0, 30)
        )
        pgx.draw.rect(screen, (200, 0, 30), ["left.385", 100, 1150, 30])
        armorfraction = self.getArmor() / self.getTotalArmor()
        pgx.draw.rect(
            screen, (110, 0, 30), ["left.385", 100, int(1150 * armorfraction), 30]
        )
    ...
    def update(self, screen, object_list, self_loc):
        ...
        if self.timer >= 0:
            ...
            AITools.shoot(
                ...
                object_list,
                self_loc,
                object_list[self_loc + 5].getRotation() + 90,
                122,
            )
            AITools.shoot(
                xpos,
                ...
                object_list,
                self_loc,
                object_list[self_loc + 5].getRotation() + 180,
                122,
            )
            AITools.shoot(
                xpos,
                ...
                object_list,
                self_loc,
                object_list[self_loc + 5].getRotation() + 270,
                122,
            )
            self.timer = -300
...
def particlemaker(xpos, ypos, xmom, ymom):
    ...
    for i in range(random.randint(max_particles - max_deviation, max_particles)):
        printerlist_add += [
            xpos,
            ypos,
            xmom + ((random.randint(-20, 20)) / random_factor),
            ymom + ((random.randint(-20, 20)) / random_factor),
            4,
            RotationState(-1, -1),
            "NA",
            particle_lifespan
            + random.randint(-lifespan_randomness, lifespan_randomness),
        ]
...
def asteroidspeedmaker(max_asteroid_spd):
    ...
    if random.randint(0, 1) == 1:
        asteroid_speedset.append(
            random.randint(int(max_asteroid_spd / 2), int(max_asteroid_spd)) / 100
        )
    else:
        asteroid_speedset.append(
            random.randint(int(-max_asteroid_spd), int(-max_asteroid_spd / 2)) / 100
        )
    if random.randint(0, 1) == 1:
        asteroid_speedset.append(
            random.randint(int(max_asteroid_spd / 2), int(max_asteroid_spd)) / 100
...
def leveler(
    ...
    for i in range(repetitions):
        ...
        # creating the entity
        asteroid_speedset = asteroidspeedmaker(
            max_asteroid_spd
        )  # getting reasonable speed
        if ID == 120:  # special for drone creation
            object_list_add = [
                ...
                0,
                ID,
                RotationState(random.randint(0, 360), 0),
                DroneAI(),
                1,
            ]
            ...
            object_list_add = [
                ...
                asteroid_speedset[1],
                ID,
                RotationState(random.randint(0, 360), random.randint(-3, 3)),
                "NA",
                1,
            ]
...
def sectorDestinations(sectornum):
    ...
    if sectornum in sectorDestData:
        ...
        return [
            ...
            1,
            1,
        ]  # in case you go into a sector that doesn't exist, it will redirect you to the home sector
    ...
    if object_list != ["PLEASE GENERATE"]:
        ...
        for i in range(int(len(object_list) / 8)):
            if object_list[4 + i * 8] == 2 or object_list[4 + i * 8] == 8:
                object_list[
                    7 + i * 8
                ] = -10  # gets rid of shots and alien shots when entering a sector
    return object_list
